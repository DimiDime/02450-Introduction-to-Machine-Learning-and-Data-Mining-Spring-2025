...
## Intro
"""
The overall goal is to understand if we can use these mathematical distance measures to:

1. Quantify how similar or different wines are from each other

2. See if wines of the same type (red or white) are more similar 
to each other than to wines of the other type

3. Potentially use these distances as a way to automatically classify wines as red or white

This is a fundamental concept in machine learning, where we often need to measure how similar 
or different data points are from each other to make predictions or classifications.
...
pairwise_distances_L1 = pairwise_distances(X, metric='manhattan')  # L1 norm
pairwise_distances_L2 = pairwise_distances(X, metric='euclidean')  # L2 norm
pairwise_distances_Linf = pairwise_distances(X, metric='chebyshev')  # L-infinity norm
...
plt.show()
#%%
pairwise_distances_L1
#%%
## TASK I: Compute the following distances and store them in the approiate variables: 
#
...
# above
#
# Question 1: Describe how the informaton about average inter and intra distances 
# can be used in (automatically) disciminating between white and red wines?
#
# Question 2: Does it make a difference if you use the L1, L2 or Linf norm? Consider the
...
# TASK: INSERT YOUR CODE HERE
def calculate_wine_distances(distance_matrix, y):
    # Find indices for red and white wines
    red_indices = np.where(y == 0)[0] 
    white_indices = np.where(y == 1)[0]
    
    # Calculate average distances
    # For red wines
    red_distances = distance_matrix[red_indices][:, red_indices]
    avg_red = np.sum(red_distances) / (red_distances.size - len(red_indices))  # exclude diagonal
    
    # For white wines
    white_distances = distance_matrix[white_indices][:, white_indices]
    avg_white = np.sum(white_distances) / (white_distances.size - len(white_indices))  # exclude diagonal
    
    # Between red and white
    red_to_white = distance_matrix[red_indices][:, white_indices]
    avg_red_to_white = np.mean(red_to_white)
    
    return avg_red, avg_white, avg_red_to_white

# Calculate for each distance metric
red_L1, white_L1, red_to_white_L1 = calculate_wine_distances(pairwise_distances_L1, y)
red_L2, white_L2, red_to_white_L2 = calculate_wine_distances(pairwise_distances_L2, y)
red_Linf, white_Linf, red_to_white_Linf = calculate_wine_distances(pairwise_distances_Linf, y)

print(f'L1 distances:\n  Red wines: {red_L1:.2f}\n  White wines: {white_L1:.2f}\n  Red-to-White: {red_to_white_L1:.2f}\n')
print(f'L2 distances:\n  Red wines: {red_L2:.2f}\n  White wines: {white_L2:.2f}\n  Red-to-White: {red_to_white_L2:.2f}\n')
print(f'Linf distances:\n  Red wines: {red_Linf:.2f}\n  White wines: {white_Linf:.2f}\n  Red-to-White: {red_to_white_Linf:.2f}\n')



'''
Q1. Average distance showcases that the red wine attributes are closer to other red wines,
where white wine attributes are closer to white wines.


Q2. White wines have the largest inbetween distance. Red-wines have the lowest.
Key Insights:
- Red wines are more homogeneous (consistent) in their characteristics across all distance metrics
- White wines show more variation in their properties
- The differences between red and white wines are substantial but not dramatically larger than the variation within white wines
- This suggests that white wines might have more diverse flavor profiles and characteristics compared to red wines in your dataset
'''
